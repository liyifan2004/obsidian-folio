# 项目文件结构

## 核心文件
- `main.ts` - 插件入口 (2814 bytes)
- `main.js` - 编译后的插件 (51007 bytes)
- `manifest.json` - 插件清单 (326 bytes)
- `styles.css` - 样式文件 (8283 bytes)

## 源代码 (src/)
- `src/followMode.ts` - 跟随模式核心逻辑
- `src/settings.ts` - 设置页面 UI
- `src/i18n.ts` - 多语言支持 (9 种语言)
- `src/types.ts` - 类型定义

## 文档
- `README.md` - 英文文档 (主文档)
- `docs/i18n/README.zh.md` - 简体中文 (6654 bytes)
- `docs/i18n/README.zh-TW.md` - 繁體中文 (2988 bytes)
- `docs/i18n/README.ja.md` - 日本語 (4867 bytes)
- `docs/i18n/README.ko.md` - 한국어 (2539 bytes)
- `docs/i18n/README.fr.md` - Français (2721 bytes)
- `docs/i18n/README.de.md` - Deutsch (2568 bytes)
- `docs/i18n/README.es.md` - Español (2732 bytes)
- `docs/i18n/README.ar.md` - العربية (3180 bytes)

## 支持的 9 种语言
1. 🇺🇸 English (默认)
2. 🇨🇳 简体中文
3. 🇹🇼 繁體中文
4. 🇯🇵 日本語
5. 🇰🇷 한국어
6. 🇫🇷 Français
7. 🇩🇪 Deutsch
8. 🇪🇸 Español
9. 🇸🇦 العربية

## 总计
- 构建产物: 51,007 bytes
- 文档: ~38,000 bytes
