...
export class DualPaneSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		modeList.createEl('li').innerHTML = '<strong>独立双栏视图</strong>：纯阅读模式，左右两栏显示接续内容，支持翻页。';
		modeList.createEl('li').innerHTML = '<strong>跟随模式</strong>：左侧为 Obsidian 原生编辑器（可编辑），右侧自动跟随显示接续内容。';
		...
		desc.createEl('p', { text: '• 跟随模式：左侧可编辑，右侧自动跟随滚动' });
		desc.createEl('p', { text: '• 独立双栏视图：纯阅读模式，支持翻页' });
